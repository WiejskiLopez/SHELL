...
## 5. Port jako ABC vs Protocol

| Kryterium | ABC | Protocol |
|-----------|-----|----------|
| Dziedziczenie | Wymagane | Nie wymagane (structural typing) |
| Testowanie | Łatwiejsze (instance check) | Elastyczniejsze |
| Zależności od frameworka | Nie | Nie |
| Kiedy użyć | Repozytoria (wymagane metody) | Proste serwisy (Clock, IdGenerator) |

## 6. Anti-Corruption Layer — Adapter dla Systemu Zewnętrznego
...
# Adapter ACL — chroni domenę przed modelem Stripe
class StripePaymentAdapter:
    """Anti-Corruption Layer między domeną a Stripe API."""
    
    async def charge(self, payment: Payment) -> PaymentResult:
        try:
            # Mapowanie domena → Stripe
            ...
            # Mapowanie Stripe → domena
            return PaymentResult(
                transaction_id=stripe_payment.id,
                status=PaymentStatus.from_stripe(stripe_payment.status),
...
- [ ] Adapter może mapować między modelami (ACL)
- [ ] Port jest stabilny — zmiany przez dodawanie, nie modyfikację
- [ ] Adapter testowany integracyjnie
- [ ] Domenowy kod używający portu testowany z mockiem/InMemory
- [ ] Jeden adapter na jeden zewnętrzny zasób
- [ ] Brak wycieków zewnętrznego modelu do domeny
