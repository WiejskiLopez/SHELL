...
def main():
    ...
    files = sorted(
        [p for p in ROOT.rglob('*') if p.is_file() and 'graph_node' in p.name and not should_exclude(p)],
    ...
    dir_count = 0
    dirs = sorted(
        [p for p in ROOT.rglob('*') if p.is_dir() and 'graph_node' in p.name and not should_exclude(p)],
        key=lambda p: len(p.parts),
        reverse=True
    )
    for d in dirs:
