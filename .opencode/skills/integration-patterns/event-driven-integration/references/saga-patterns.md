...
class CreateInvoiceOnOrderPlacedHandler:
    async def handle(self, order_placed_event: OrderPlacedEvent) -> None:
        async with self._unit_of_work as unit_of_work:
            if await unit_of_work.inbox.contains(order_placed_event.event_id):
                return
            invoice = Invoice.create(
                order_id=order_placed_event.order_id,
                amount=order_placed_event.total_amount,
                customer_id=order_placed_event.customer_id,
            )
...
class ReserveStockOnInvoiceCreatedHandler:
    async def handle(self, invoice_created_event: InvoiceCreatedEvent) -> None:
        async with self._unit_of_work as unit_of_work:
            if await unit_of_work.inbox.contains(invoice_created_event.event_id):
                return
            # ... rezerwacja stocku ...
            await unit_of_work.inbox.add(event.event_id)
...
class ReleaseStockOnPaymentFailedHandler:
    async def handle(self, payment_failed_event: PaymentFailedEvent) -> None:
        async with self._unit_of_work as unit_of_work:
            if await unit_of_work.inbox.contains(payment_failed_event.event_id):
                return
            inventory = await unit_of_work.inventories.get_by_order_id(payment_failed_event.order_id)
            inventory.release(payment_failed_event.order_id)
            await unit_of_work.inbox.add(payment_failed_event.event_id)
...
class ReleaseStockHandler:
    async def handle(self, release_stock_command: ReleaseStockCommand) -> None:
        async with self._unit_of_work as unit_of_work:
            inventory = await unit_of_work.inventories.get_by_order_id(release_stock_command.order_id)
            if inventory.is_already_released(release_stock_command.order_id):
                return  # idempotencja — już zwolnione
            inventory.release(release_stock_command.order_id)
