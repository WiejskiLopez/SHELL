...
from shell.application.queries.queries import GetNodeResultQuery
...
async def get_node_result(
    ...
    result = await query_bus.dispatch(GetNodeResultQuery(node_id=node_id, workflow_id=workflow_id))
    if result is None:
