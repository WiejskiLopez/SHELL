...
<#
.SYNOPSIS
    Zarzadza kontenerami mikroserwisu session (api + worker) w projekcie shell-dev.
...
.DESCRIPTION
    ...
    Przyklady:
        .\docker\scripts\manage.ps1 up       # start kontenerow session
        .\docker\scripts\manage.ps1 restart  # restart kontenerow session
...
$targetServices = @("shell-session-api", "shell-session-worker")
$containerNames = @("shell-session-shell-session-api-1", "shell-session-shell-session-worker-1")

Write-Host "session [$Action] ($Environment)" -ForegroundColor Cyan
...
switch ($Action) {
    ...
    "redeploy" { Remove-StaleContainers; & (Join-Path $projectRoot "scripts\prepare_mtls_ca.ps1"); if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; $env:CERTIFICATE_BUILD_ID = "session-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmssfff'))-$PID"; & docker compose @composeArgs up -d --build @targetServices 2>&1 | Out-Host; exit $LASTEXITCODE }
