...
class TestGraphNodeExecutionWorkerHappyPath:
    async def test_first_node_success_records_result_and_does_not_advance(self) -> None:
        ...
        task_execution, graph_execution = _build_graph_execution(unit_of_work, "happy", ["agent", "tool"])
    ...
    async def test_last_node_success_records_result_and_does_not_finish(self) -> None:
        unit_of_work = InMemoryUnitOfWork()
        task_execution, graph_execution = _build_graph_execution(unit_of_work, "single", ["agent"])
        wf = await _persist_running_workflow(
            unit_of_work, task_execution.id, graph_execution.graph_node_executions[0].id
        )
