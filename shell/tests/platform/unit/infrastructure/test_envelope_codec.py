...
class TestEnvelopeCodec:
    def test_round_trip_preserves_fields(self) -> None:
        ...
        assert decoded.integration_event_name == "TaskExecutionCreatedEvent"
    ...
    def test_integration_event_name_is_a_wire_envelope_field(self) -> None:
        encoded = EnvelopeCodec().encode(_envelope())

        assert b'"integration_event_name":"TaskExecutionCreatedEvent"' in encoded
        assert b'"command_name"' not in encoded
    ...
    def test_decode_rejects_unknown_kind(self) -> None:
        ...
        try:
            ...
            assert "event delivery" in str(exc)
        else:
