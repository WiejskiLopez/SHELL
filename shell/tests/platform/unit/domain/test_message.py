"""Unit tests for Message aggregate."""
...
class TestMessage:
    ...
    def test_new_generates_message_created_event(self) -> None:
        ...
        assert event.message_type.value == "test.event"
    ...
    def test_mark_as_received_emits_event(self) -> None:
        ...
        assert len(events) == 1
        event = events[0]
        assert event.previous_status == MessageStatus.CREATED
        assert event.new_status == MessageStatus.RECEIVED
