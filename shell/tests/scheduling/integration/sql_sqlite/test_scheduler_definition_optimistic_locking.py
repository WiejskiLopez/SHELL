"""Optimistic locking tests for SchedulerDefinition aggregate."""
...
class TestSchedulerDefinitionOptimisticLocking:
    async def test_concurrent_modification_raises_error(
        ...
        async with uow1 as u:
            entity = SchedulerDefinition(id=sd_id, name="v1")
            await u.scheduler_definition_repository.save(entity)
            await u.commit()

        async with uow1 as u1:
            entity_a = await u1.scheduler_definition_repository.get_by_id(sd_id)
            assert entity_a is not None

            async with uow2 as u2:
                entity_b = await u2.scheduler_definition_repository.get_by_id(sd_id)
                assert entity_b is not None

                modified_a = SchedulerDefinition(id=sd_id, name="v2")
                await u1.scheduler_definition_repository.save(modified_a)
                await u1.commit()

                modified_b = SchedulerDefinition(id=sd_id, name="v3")
                await u2.scheduler_definition_repository.save(modified_b)
