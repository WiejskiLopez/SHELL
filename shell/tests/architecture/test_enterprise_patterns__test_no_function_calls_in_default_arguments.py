"""Koncept: reguła architektoniczna dotycząca enterprise patterns: test no function calls in default arguments.

Reguła: test sprawdza kontrakt architektoniczny enterprise patterns: test no function calls in default arguments.
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                for default in node.args.defaults + node.args.kw_defaults:
                    if default is not None and isinstance(default, ast.Call) and (not _is_fastapi_sentinel(default)):
                        ...
                        violations.append(f'{rel}:{node.lineno}: {node.name} — wywołanie w default arg')
    assert not violations, architecture_assertion_message('reguła testowana przez test_no_function_calls_in_default_arguments', 'warunek zapisany w asercji musi być spełniony', '\n'.join(violations))
