...
def test_app_mappers_have_to_dto() -> None:
    ...
    for mapper_path in (BASE / 'application').rglob('**/mappers/**/*.py'):
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name.endswith('_to_dto'):
                has_to_dto = True
                break
        if not has_to_dto:
            violations.append(f'{mapper_path.relative_to(BASE)}: missing *_to_dto function')
    assert not violations, architecture_assertion_message('reguła testowana przez test_app_mappers_have_to_dto', 'warunek zapisany w asercji musi być spełniony', 'Application mappers must have at least a *_to_dto function:\n' + '\n'.join(violations))
