"""Koncept: reguła architektoniczna dotycząca domain structure: test repository port signatures have domain types.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test repository port signatures have domain types.
                ...
                for stmt in node.body:
                    ...
                    if stmt.returns:
                        ...
                        if primitive:
                            ...
                            violations.append(key)
    assert not violations, architecture_assertion_message('reguła testowana przez test_repository_port_signatures_have_domain_types', 'warunek zapisany w asercji musi być spełniony', 'Repository port methods must use domain types in their signatures, not primitives.\nParameters and return types should be ValueObjects, IDs, or domain aggregates:\n' + '\n'.join(violations))
