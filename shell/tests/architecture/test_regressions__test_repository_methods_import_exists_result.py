...
def test_repository_methods_import_exists_result() -> None:
    ...
    for py_file in iter_py_files(BASE / 'shell' / 'infrastructure'):
        ...
        if tree is None:
            continue
        has_import = any(isinstance(n, ast.ImportFrom) and n.module and any(alias.name == 'ExistsResult' for alias in n.names) for n in ast.walk(tree) if isinstance(n, (ast.Import, ast.ImportFrom)))
        key = str(rel)
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_repository_methods_import_exists_result', 'warunek zapisany w asercji musi być spełniony', 'Repository files that define exists() must import ExistsResult:\n' + '\n'.join(violations))
