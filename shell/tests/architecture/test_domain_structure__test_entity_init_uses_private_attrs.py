"""Koncept: reguła architektoniczna dotycząca domain structure: test entity init uses private attrs.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test entity init uses private attrs.
            ...
            for stmt in node.body:
                if isinstance(stmt, ast.FunctionDef) and stmt.name == '__init__':
                    for line in ast.walk(stmt):
                        if isinstance(line, ast.Attribute) and isinstance(line.value, ast.Name) and (line.value.id == 'self') and (not line.attr.startswith('_')):
                            key = f'{path.relative_to(BASE)}: {node.name}.__init__ assigns to public {line.attr!r}'
                            if key not in _KNOWN_PUBLIC_INIT_ATTRS:
                                violations.append(key)
    assert not violations, architecture_assertion_message('reguła testowana przez test_entity_init_uses_private_attrs', 'warunek zapisany w asercji musi być spełniony', 'Entity/AggregateRoot __init__ must assign to private attributes with _ prefix:\n' + '\n'.join(violations))
