"""Koncept: reguła architektoniczna dotycząca domain structure: test collections returned as copies.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test collections returned as copies.
...
def test_collections_returned_as_copies() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_collections_returned_as_copies', 'warunek zapisany w asercji musi być spełniony', 'Properties returning collections should return copies (use .copy(), list(...), tuple(...), [:]):\n' + '\n'.join(violations))
