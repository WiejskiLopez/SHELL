...
def test_domain_event_fields_no_mutable_collections() -> None:
    ...
    for path in _iter_domain_files():
        ...
        for node in ast.walk(tree):
            ...
            for stmt in node.body:
                ...
                if field_name in ('event_id', 'aggregate_id', 'aggregate_name', 'occurred_at', 'correlation_id', 'causation_id', 'schema_version', 'kind'):
                    continue
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_domain_event_fields_no_mutable_collections', 'warunek zapisany w asercji musi być spełniony', 'DomainEvent fields should not use mutable collection types (list/dict/set). Use tuple/frozenset/Sequence instead. Known violations in _KNOWN_EVENT_MUTABLE_FIELDS:\n' + '\n'.join(violations))
