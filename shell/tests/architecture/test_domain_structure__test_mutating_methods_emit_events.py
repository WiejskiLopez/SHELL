"""Koncept: reguła architektoniczna dotycząca domain structure: test mutating methods emit events.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test mutating methods emit events.
...
def test_mutating_methods_emit_events() -> None:
    violations: list[str] = []
    _NON_MUTATING = frozenset({'__init__', 'restore', 'pull_events', 'to_dict', 'new', 'create', 'generate', 'matches_trigger', 'get', 'snapshot', 'create_main_round', 'create_sub_graph', 'initialize', 'open'})
            ...
            for stmt in node.body:
                ...
                if not (emits_directly or delegates):
                    ...
                    if key not in _KNOWN_NO_EVENT_EMIT:
                        violations.append(f'{key} does not emit an event (no append_event/_change/_delete)')
    assert not violations, architecture_assertion_message('reguła testowana przez test_mutating_methods_emit_events', 'warunek zapisany w asercji musi być spełniony', 'Public mutating methods in aggregates must emit an event (append_event or delegate to _change/_delete):\n' + '\n'.join(violations))
