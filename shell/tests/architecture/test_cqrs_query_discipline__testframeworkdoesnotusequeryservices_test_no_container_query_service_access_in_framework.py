"""Koncept: reguła architektoniczna dotycząca cqrs query discipline: TestFrameworkDoesNotUseQueryServices test no container query service access in framework.

Reguła: test sprawdza kontrakt architektoniczny cqrs query discipline: TestFrameworkDoesNotUseQueryServices test no container query service access in framework.
    ...
    def test_no_container_query_service_access_in_framework(self) -> None:
        ...
        for py_file in iter_py_files(SHELL_SRC):
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and (node.func.id == 'getattr') and node.args and isinstance(node.args[1], ast.Constant) and isinstance(node.args[1].value, str) and node.args[1].value.endswith('_query_service'):
                    violations.append(f'{_rel(py_file)} getattr({node.args[1].value})')
        assert not violations, architecture_assertion_message('reguła testowana przez test_no_container_query_service_access_in_framework', 'warunek zapisany w asercji musi być spełniony', 'Framework reaches QueryService through the container:\n' + '\n'.join(violations))
