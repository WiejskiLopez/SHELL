"""Koncept: reguła architektoniczna dotycząca domain structure: test domain event fields are ids only.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test domain event fields are ids only.
...
def test_domain_event_fields_are_ids_only() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_domain_event_fields_are_ids_only', 'warunek zapisany w asercji musi być spełniony', 'DomainEvent fields are a closed contract: only ID fields (suffix `_id`) and `occurred_at` are allowed. Do not add other fields:\n' + '\n'.join(violations))
