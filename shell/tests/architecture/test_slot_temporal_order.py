...
def test_slots_temporal_fields_first() -> None:
    ...
    for path in iter_domain_files():
        ...
        for node in find_classes(tree):
            ...
            if temporal and business and (max(slots.index(slot) for slot in temporal) > min(slots.index(slot) for slot in business)):
                violations.append(f'{path.relative_to(BASE)}:{node.lineno} {node.name}: business slots precede temporal slots: {slots}')
            ranked = [_temporal_rank(slot) for slot in temporal]
            if ranked != sorted(ranked):
                violations.append(f'{path.relative_to(BASE)}:{node.lineno} {node.name}: temporal slots are out of order: {temporal}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_slots_temporal_fields_first', 'warunek zapisany w asercji musi być spełniony', architecture_failure('czasowe pola są przed polami biznesowymi w slots agregatu', '_created_at/_occurred_at poprzedzają _changed_at, potem _deleted_at i pola biznesowe', violations, 'zmień kolejność deklaracji __slots__ agregatu'))
