"""Koncept: reguła architektoniczna dotycząca domain structure: test specifications extend specification.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test specifications extend specification.
...
def test_specifications_extend_specification() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_specifications_extend_specification', 'warunek zapisany w asercji musi być spełniony', 'Specifications must extend Specification[T]:\n' + '\n'.join(violations))
