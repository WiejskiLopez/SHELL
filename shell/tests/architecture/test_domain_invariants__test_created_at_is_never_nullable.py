    ...
    for path in iter_domain_files():
        ...
        for node in find_classes(tree):
            ...
            for stmt in node.body:
                if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef)) and stmt.name in ('__init__', 'restore') and _check_method_signature(stmt, 'created_at'):
                    key = f'{path.relative_to(BASE)}: {node.name}.{stmt.name} has nullable created_at param'
                    if key not in _KNOWN_NULLABLE_CREATED_AT:
                        violations.append(key)
            for stmt in node.body:
                if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef)) and stmt.name == 'created_at':
                    source = ast.unparse(stmt)
                    if '-> CreatedAt | None' in source:
                        ...
                        if key not in _KNOWN_NULLABLE_CREATED_AT:
                            violations.append(key)
            for stmt in node.body:
                if isinstance(stmt, ast.AnnAssign) and stmt.target and (ast.unparse(stmt.target) == '_created_at'):
                    source = ast.unparse(stmt)
                    if 'CreatedAt | None' in source:
                        key = f'{path.relative_to(BASE)}: {node.name}._created_at type is CreatedAt | None'
                        if key not in _KNOWN_NULLABLE_CREATED_AT:
                            violations.append(key)
    assert not violations, architecture_assertion_message('reguła testowana przez test_created_at_is_never_nullable', 'warunek zapisany w asercji musi być spełniony', 'created_at must NEVER be nullable (CreatedAt | None). Every aggregate always has a creation timestamp:\n' + '\n'.join(violations))
