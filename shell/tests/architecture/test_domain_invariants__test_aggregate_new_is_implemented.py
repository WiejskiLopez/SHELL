                ...
                if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef)) and stmt.name == '_new':
                    ...
                    if 'NotImplementedError' in source:
                        ...
                        if key not in _KNOWN_STUB_NEW:
                            violations.append(key)
    assert not violations, architecture_assertion_message('reguła testowana przez test_aggregate_new_is_implemented', 'warunek zapisany w asercji musi być spełniony', '_new() must be implemented in every aggregate. NotImplementedError stubs are not allowed:\n' + '\n'.join(violations))
