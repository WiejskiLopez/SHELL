        ...
        for py_file in dto_dir.rglob('*.py'):
            ...
            for node in ast.walk(tree):
                ...
                for stmt in node.body:
                    if isinstance(stmt, ast.AnnAssign) and stmt.annotation and _has_complex_type(stmt.annotation):
                        field_name = stmt.target.id if isinstance(stmt.target, ast.Name) else repr(stmt.target)
                        violations.append(f'{py_file.relative_to(BASE)}: {node.name}.{field_name}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_dto_fields_use_only_primitives', 'warunek zapisany w asercji musi być spełniony', 'DTO fields must not use datetime/Decimal/dict/list/set types (use str instead):\n' + '\n'.join(violations))
