"""Koncept: reguła architektoniczna dotycząca naming conventions: test no abbreviations in class names.

Reguła: test sprawdza kontrakt architektoniczny naming conventions: test no abbreviations in class names.
...
def test_no_abbreviations_in_class_names() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_no_abbreviations_in_class_names', 'warunek zapisany w asercji musi być spełniony', 'Class names must not use abbreviations:\n' + '\n'.join(violations))
