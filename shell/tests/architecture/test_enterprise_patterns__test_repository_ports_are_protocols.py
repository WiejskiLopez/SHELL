    ...
    for repos_dir in (BASE / 'domain').rglob('repositories'):
        ...
        for path in _iter_py_files(repos_dir):
            ...
            for node in ast.walk(tree):
                ...
                if not node.name.endswith('Repository'):
                    continue
                has_protocol = any(isinstance(b, ast.Name) and b.id in {'Protocol', 'ABC'} for b in node.bases)
                if not has_protocol:
                    violations.append(f'{path.relative_to(BASE)}: class {node.name}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_repository_ports_are_protocols', 'warunek zapisany w asercji musi być spełniony', 'Repository ports must be Protocols or ABCs:\n' + '\n'.join(violations))
