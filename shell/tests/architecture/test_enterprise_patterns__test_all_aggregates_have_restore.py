    ...
    for path in _iter_domain_files():
        ...
        for node in ast.walk(tree):
            ...
            if not any(_is_aggregate_root_base(b) for b in node.bases):
                continue
            has_restore = any(isinstance(m, ast.FunctionDef) and m.name == 'restore' for m in node.body)
            if not has_restore:
                key = f'{path.relative_to(BASE).as_posix()}: class {node.name}'
                if key not in _KNOWN_MISSING_RESTORE:
                    missing.append(key)
    assert not missing, architecture_assertion_message('reguła testowana przez test_all_aggregates_have_restore', 'warunek zapisany w asercji musi być spełniony', 'Unexpected aggregate(s) missing restore(). If this is intentional, add them to _KNOWN_MISSING_RESTORE:\n' + '\n'.join(missing))
