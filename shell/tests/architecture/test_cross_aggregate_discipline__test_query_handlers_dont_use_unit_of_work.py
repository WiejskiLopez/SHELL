...
def test_query_handlers_dont_use_unit_of_work() -> None:
    ...
    assert not violations, architecture_assertion_message(
        "reguła testowana przez test_query_handlers_dont_use_unit_of_work",
        "warunek zapisany w asercji musi być spełniony",
        "Query handlers must NOT use domain repositories. Use an injected QueryService port and return an application DTO instead:\n"
        + "\n".join(violations),
    )
