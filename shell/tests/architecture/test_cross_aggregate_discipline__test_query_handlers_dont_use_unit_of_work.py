"""Koncept: reguła architektoniczna dotycząca cross aggregate discipline: test query handlers dont use unit of work.

Reguła: test sprawdza kontrakt architektoniczny cross aggregate discipline: test query handlers dont use unit of work.
...
def test_query_handlers_dont_use_unit_of_work() -> None:
    ...
    violations: list[str] = []
    query_handler_dirs = [path for path in (BASE / 'application').rglob('query_handlers') if path.is_dir()]
    ...
    for query_handler_dir in query_handler_dirs:
        for path in iter_py_files(query_handler_dir):
            ...
            used_repositories.update(repository_name for repository_name in _find_repo_injected_in_init(tree) if repository_name in known_repositories)
            for repository_name in sorted(used_repositories):
                violations.append(f'{rel}: {repository_name}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_query_handlers_dont_use_unit_of_work', 'warunek zapisany w asercji musi być spełniony', 'Query handlers must NOT use domain repositories. Use an injected QueryService port and return an application DTO instead:\n' + '\n'.join(violations))
