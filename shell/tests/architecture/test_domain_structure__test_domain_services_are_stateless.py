"""Koncept: reguła architektoniczna dotycząca domain structure: test domain services are stateless.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test domain services are stateless.
...
def test_domain_services_are_stateless() -> None:
            ...
            for stmt in node.body:
                if isinstance(stmt, ast.FunctionDef) and stmt.name == '__init__':
                    for line in ast.walk(stmt):
                        if isinstance(line, ast.Attribute) and isinstance(line.value, ast.Name) and (line.value.id == 'self'):
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_domain_services_are_stateless', 'warunek zapisany w asercji musi być spełniony', 'Domain services must be stateless (no state beyond injected dependencies):\n' + '\n'.join(violations))
