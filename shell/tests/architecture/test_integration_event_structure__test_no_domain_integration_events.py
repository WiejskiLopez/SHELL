...
def test_no_domain_integration_events() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_no_domain_integration_events', 'warunek zapisany w asercji musi być spełniony', 'IntegrationEvent classes must live in application/, not domain/:\n' + '\n'.join(violations))
