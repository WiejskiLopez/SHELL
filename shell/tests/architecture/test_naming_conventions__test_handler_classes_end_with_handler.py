...
def test_handler_classes_end_with_handler() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_handler_classes_end_with_handler', 'warunek zapisany w asercji musi być spełniony', "Handler classes must end with 'Handler':\n" + '\n'.join(violations))
