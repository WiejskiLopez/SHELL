"""Architecture tests for bounded-context snapshot ownership."""
...
def test_snapshots_are_defined_in_a_bounded_context_domain() -> None:
    ...
    assert not violations, "Snapshot ownership violations:\n" + "\n".join(violations)
...
def test_snapshots_are_not_imported_outside_their_own_bounded_context() -> None:
            ...
            if isinstance(node, ast.ImportFrom) and node.module:
                ...
                for alias in node.names:
                    ...
                    if target_bc is not None and target_bc != source_bc:
                        violations.append(
                            f"{path.relative_to(BASE).as_posix()}: imports snapshot module "
                            f"{imported_module!r} owned by BC {target_bc}"
                        )
                continue
    ...
    assert not violations, "Snapshots must not cross bounded-context boundaries:\n" + "\n".join(
        violations
    )
