...
def test_process_handlers_are_stateless() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_process_handlers_are_stateless', 'warunek zapisany w asercji musi być spełniony', 'Process handlers must declare dependencies via constructor injection:\n' + '\n'.join(violations))
