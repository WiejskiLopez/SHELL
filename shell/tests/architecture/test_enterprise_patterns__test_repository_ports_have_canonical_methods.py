...
def test_repository_ports_have_canonical_methods() -> None:
    ...
    for repos_dir in (BASE / 'domain').rglob('repositories'):
        ...
        for path in _iter_py_files(repos_dir):
            ...
            for node in ast.walk(tree):
                ...
                if not node.name.endswith('Repository'):
                    continue
                defined = {m.name for m in node.body if isinstance(m, (ast.FunctionDef, ast.AsyncFunctionDef))}
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_repository_ports_have_canonical_methods', 'warunek zapisany w asercji musi być spełniony', 'Repository ports should define save/get_by_id/exists/delete.\nIf a method is intentionally absent, add it to _KNOWN_MISSING_REPO_METHODS:\n' + '\n'.join(violations))
