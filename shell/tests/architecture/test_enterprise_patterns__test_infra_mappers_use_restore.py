...
def test_infra_mappers_use_restore() -> None:
    ...
    for path in _iter_domain_files():
        tree = ast.parse(path.read_text(encoding='utf-8'))
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef) and any(_is_aggregate_root_base(b) for b in node.bases):
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_infra_mappers_use_restore', 'warunek zapisany w asercji musi być spełniony', 'Infrastructure mappers should call Aggregate.restore(), not Aggregate(...). Add known violations to _KNOWN_MAPPER_USES_INIT:\n' + '\n'.join(violations))
