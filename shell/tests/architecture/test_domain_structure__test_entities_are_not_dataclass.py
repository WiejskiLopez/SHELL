"""Koncept: reguła architektoniczna dotycząca domain structure: test entities are not dataclass.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test entities are not dataclass.
...
def test_entities_are_not_dataclass() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_entities_are_not_dataclass', 'warunek zapisany w asercji musi być spełniony', 'Entities/AggregateRoots must NOT be @dataclass (identity-based equality):\n' + '\n'.join(violations))
