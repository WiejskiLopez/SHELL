...
def test_domain_services_do_not_import_infrastructure() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_domain_services_do_not_import_infrastructure', 'warunek zapisany w asercji musi być spełniony', 'Domain services must not import from infrastructure/:\n' + '\n'.join(violations))
