...
def test_cross_bc_imports_only_in_infrastructure() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_cross_bc_imports_only_in_infrastructure', 'warunek zapisany w asercji musi być spełniony', 'Cross-BC dependencies must use public HTTP/event contracts, not implementation imports:\n' + '\n'.join(violations))
