...
def test_aggregate_factory_delegates_to_new_and_emits_created() -> None:
    ...
    for path in _iter_domain_files():
        ...
        for node in find_classes(tree):
            ...
            if private_new is None:
                violations.append(
                    f"{rel}: {node.name}.{factory_name}() — aggregate factory must delegate to _new(), but _new() is missing"
                )
                continue
    ...
    assert not violations, architecture_assertion_message(
        "reguła testowana przez test_aggregate_factory_delegates_to_new_and_emits_created",
