"""Koncept: reguła architektoniczna dotycząca domain structure: test aggregate factory delegates to new and emits created.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test aggregate factory delegates to new and emits created.
    ...
    for path in _iter_domain_files():
        ...
        for node in find_classes(tree):
            ...
            if node.name in _NON_FACTORY_AGGREGATES:
                continue
            factory = next((_classmethod_source(node, name) for name in _AGGREGATE_FACTORY_METHODS if _classmethod_source(node, name) is not None), None)
            if factory is None:
                continue
            factory_name = factory.name
            private_new = _classmethod_source(node, '_new')
            if private_new is None:
                violations.append(f'{rel}: {node.name}.{factory_name}() — aggregate factory must delegate to _new(), but _new() is missing')
                continue
            if not _calls_new(factory):
                violations.append(f'{rel}: {node.name}.{factory_name}() — factory must delegate to _new()')
            if not _emits_created_event(private_new):
                violations.append(f'{rel}: {node.name}._new() — must call append_event(*CreatedEvent)')
    assert not violations, architecture_assertion_message('reguła testowana przez test_aggregate_factory_delegates_to_new_and_emits_created', 'warunek zapisany w asercji musi być spełniony', 'Aggregate factories must delegate to _new(), and _new() must emit a *CreatedEvent:\n' + '\n'.join(violations))
