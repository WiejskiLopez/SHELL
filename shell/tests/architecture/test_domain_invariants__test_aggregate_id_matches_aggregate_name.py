            ...
            for base in node.bases:
                ...
                if id_type_name and id_type_name.endswith('Id') and (id_type_name != expected_id):
                    ...
                    if key not in _KNOWN_AGGREGATE_ID_MISMATCH:
                        violations.append(key)
    assert not violations, architecture_assertion_message('reguła testowana przez test_aggregate_id_matches_aggregate_name', 'warunek zapisany w asercji musi być spełniony', 'Aggregate ID must match its name (e.g. User -> UserId). Rename the ID class or change which ID the aggregate uses:\n' + '\n'.join(violations))
