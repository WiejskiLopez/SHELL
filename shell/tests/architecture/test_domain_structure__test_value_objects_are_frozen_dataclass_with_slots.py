"""Koncept: reguła architektoniczna dotycząca domain structure: test value objects are frozen dataclass with slots.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test value objects are frozen dataclass with slots.
...
def test_value_objects_are_frozen_dataclass_with_slots() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_value_objects_are_frozen_dataclass_with_slots', 'warunek zapisany w asercji musi być spełniony', 'ValueObjects must be @dataclass(frozen=True, slots=True):\n' + '\n'.join(violations))
