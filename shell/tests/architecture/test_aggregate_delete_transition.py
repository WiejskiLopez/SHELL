...
def test_aggregate_delete_sets_deleted_at_and_emits_event() -> None:
    ...
    for path in iter_domain_files():
        ...
        for node in find_classes(tree):
            ...
            for statement in node.body:
                if isinstance(statement, (ast.FunctionDef, ast.AsyncFunctionDef)) and statement.name == '_delete':
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_aggregate_delete_sets_deleted_at_and_emits_event', 'warunek zapisany w asercji musi być spełniony', architecture_failure('przejście usunięcia agregatu publikuje zmianę stanu', '_delete() ustawia _deleted_at i wywołuje append_event()', violations, 'uzupełnij przejście usunięcia agregatu przed powrotem z metody'))
