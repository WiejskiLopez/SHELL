    ...
    for layer in _CHECK_LAYERS:
        for path in iter_py_files(BASE / layer):
            ...
            for i, line in enumerate(content.splitlines(), 1):
                stripped = line.strip()
                if stripped.startswith('#') and '# noqa' not in stripped and (not stripped.startswith('#!')) and (not stripped.startswith('# -*-')) and re.match('# \\w', stripped):
                    violations.append(f'{rel}:{i}: {stripped[:80]}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_no_comments_in_production_code', 'warunek zapisany w asercji musi być spełniony', 'Domain/application code should avoid comments (except # noqa):\n' + '\n'.join(violations))
