...
def test_mappers_have_no_business_logic() -> None:
    violations: list[str] = []
    for mapper_path in list((BASE / 'infrastructure').rglob('**/mappers/**/*.py')) + list((BASE / 'application').rglob('**/mappers/**/*.py')):
                    ...
                    if isinstance(sub, ast.If):
                        ...
                        if '==' in test_src or 'is' in test_src:
                            ...
                            if not any(kw in line for kw in ['None', 'is not None', 'is None', 'isinstance', 'type(']):
                                violations.append(f'{mapper_path.relative_to(BASE)}: {node.name} has business logic at line {sub.lineno}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_mappers_have_no_business_logic', 'warunek zapisany w asercji musi być spełniony', 'Mapper functions should contain no business logic (no if/elif with data checks):\n' + '\n'.join(violations))
