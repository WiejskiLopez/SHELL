...
def test_all_repository_ports_have_in_memory() -> None:
    ...
    for file_path, class_name in repos:
        ...
        found = any(candidate.is_file() for candidate in (BASE / 'infrastructure').rglob(expected_pat))
        if not found:
            missing.append(f'{file_path.relative_to(BASE)}: {class_name}')
    assert not missing, architecture_assertion_message('reguła testowana przez test_all_repository_ports_have_in_memory', 'warunek zapisany w asercji musi być spełniony', 'Repository ports must have a corresponding InMemory implementation:\n' + '\n'.join(missing))
