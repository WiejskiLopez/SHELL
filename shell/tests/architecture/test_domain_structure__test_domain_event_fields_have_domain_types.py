"""Koncept: reguła architektoniczna dotycząca domain structure: test domain event fields have domain types.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test domain event fields have domain types.
...
def test_domain_event_fields_have_domain_types() -> None:
        ...
        for node in find_classes(tree):
            ...
            for stmt in node.body:
                ...
                if not isinstance(stmt.target, ast.Name):
                    continue
                name = stmt.target.id
                if name in ('event_id', 'aggregate_id', 'aggregate_name', 'occurred_at', 'correlation_id', 'causation_id', 'schema_version', 'kind', 'key', 'old_value', 'new_value'):
                    continue
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_domain_event_fields_have_domain_types', 'warunek zapisany w asercji musi być spełniony', 'DomainEvent fields must use ValueObjects, not primitives (str/dict/list/Any):\n' + '\n'.join(violations))
