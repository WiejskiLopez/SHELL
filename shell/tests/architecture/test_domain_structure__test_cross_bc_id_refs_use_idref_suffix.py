"""Koncept: reguła architektoniczna dotycząca domain structure: test cross bc id refs use idref suffix.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test cross bc id refs use idref suffix.
...
def test_cross_bc_id_refs_use_idref_suffix() -> None:
    ...
    for stem, entries in id_classes_by_stem.items():
        ...
        for _, loc in entries:
            violations.append(f"{loc} — duplicate ID stem '{stem}' found in BCs {bcs}. Only one BC may own {{stem}}Id; others must use {{stem}}IdRef.")
    assert not violations, architecture_assertion_message('reguła testowana przez test_cross_bc_id_refs_use_idref_suffix', 'warunek zapisany w asercji musi być spełniony', 'Duplicate *Id class names across Bounded Contexts — use IdRef suffix for cross-BC references:\n' + '\n'.join(violations))
