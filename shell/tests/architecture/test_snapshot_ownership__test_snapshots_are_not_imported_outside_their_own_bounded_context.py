...
def _production_python_files() -> list[Path]:
    return [path for path in BASE.rglob('*.py') if not any(excluded in path.parts for excluded in {'tests', '.venv', '__pycache__', '.pytest_cache', '.opencode'})]
...
def _snapshot_modules() -> dict[str, str]:
    ...
    for path in _production_python_files():
        ...
        has_snapshot_symbol = any(isinstance(node, ast.ClassDef) and node.name.endswith('Snapshot') for node in ast.walk(tree))
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.ImportFrom) and node.module:
                ...
                for alias in node.names:
                    ...
                    target_bc = next((owner for module, owner in snapshot_modules.items() if imported_module == module or imported_module.startswith(module + '.')), None)
                    if target_bc is not None and target_bc != source_bc:
                        violations.append(f'{path.relative_to(BASE).as_posix()}: imports snapshot module {imported_module!r} owned by BC {target_bc}')
                continue
            ...
            if target_bc is not None and target_bc != source_bc:
                violations.append(f'{path.relative_to(BASE).as_posix()}: imports snapshot module {imported_module!r} owned by BC {target_bc}')
    assert not violations, architecture_assertion_message('reguła testowana przez test_snapshots_are_not_imported_outside_their_own_bounded_context', 'warunek zapisany w asercji musi być spełniony', 'Snapshots must not cross bounded-context boundaries:\n' + '\n'.join(violations))
