...
def test_mypy_domain_and_application_zero_errors() -> None:
    ...
    layer_paths = [path for bc_path in SHELL_PKG.iterdir() if bc_path.is_dir() for path in (bc_path / 'domain', bc_path / 'application') if path.exists()]
    result = subprocess.run([sys.executable, '-m', 'mypy', '--disable-error-code=type-abstract', *[str(path) for path in layer_paths]], capture_output=True, text=True, cwd=str(PROJECT_ROOT))
