"""Koncept: reguła architektoniczna dotycząca domain structure: test entities have slots.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test entities have slots.
...
def test_entities_have_slots() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_entities_have_slots', 'warunek zapisany w asercji musi być spełniony', 'Entities/AggregateRoots must define __slots__:\n' + '\n'.join(violations))
