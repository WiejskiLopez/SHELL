"""Koncept: reguła architektoniczna dotycząca domain structure: test domain events are frozen dataclass.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test domain events are frozen dataclass.
...
def test_domain_events_are_frozen_dataclass() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_domain_events_are_frozen_dataclass', 'warunek zapisany w asercji musi być spełniony', 'DomainEvents must be @dataclass(frozen=True):\n' + '\n'.join(violations))
