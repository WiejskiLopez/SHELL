...
def test_each_bc_has_distinct_metadata_with_only_its_tables() -> None:
    ...
    for bounded_context, metadata in metadata_by_bc.items():
        assert {"inbox_event", "outbox_event"}.issubset(metadata.tables), (
            architecture_assertion_message(
                ...
                f"Naruszona reguła: każdy BC musi rejestrować własne tabele delivery. Znaleziono brak tabel w BC {bounded_context!r}: { {'inbox_event', 'outbox_event'} - set(metadata.tables) }. Jak naprawić: zbuduj modele delivery na bazie SQLAlchemy tego BC.",
            )
        )
    assert len({id(metadata) for metadata in metadata_by_bc.values()}) == len(_BASES), (
        architecture_assertion_message(
            "reguła testowana przez test_each_bc_has_distinct_metadata_with_only_its_tables",
            "warunek zapisany w asercji musi być spełniony",
            "Naruszona reguła: każdy BC musi mieć odrębny obiekt MetaData. Znaleziono współdzielone metadane. Jak naprawić: nie używaj jednej deklaratywnej bazy dla wielu BC.",
        )
    )
