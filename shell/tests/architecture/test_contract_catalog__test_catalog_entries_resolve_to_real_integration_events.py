...
def test_catalog_entries_resolve_to_real_integration_events() -> None:
    ...
    for bounded_context in _CATALOG_MODULES:
        ...
        for entry in catalog.entries:
            assert entry.type_name in registry, architecture_assertion_message('reguła testowana przez test_catalog_entries_resolve_to_real_integration_events', 'warunek zapisany w asercji musi być spełniony', f'{bounded_context}: catalog entry {entry.type_name} has no registered type')
            resolved = registry[entry.type_name]
            assert issubclass(resolved, base), architecture_assertion_message('reguła testowana przez test_catalog_entries_resolve_to_real_integration_events', 'warunek zapisany w asercji musi być spełniony', f'{bounded_context}: {entry.type_name} must inherit IntegrationEvent')
