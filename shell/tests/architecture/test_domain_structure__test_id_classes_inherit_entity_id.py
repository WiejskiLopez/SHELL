"""Koncept: reguła architektoniczna dotycząca domain structure: test id classes inherit entity id.

Reguła: test sprawdza kontrakt architektoniczny domain structure: test id classes inherit entity id.
...
def test_id_classes_inherit_entity_id() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_id_classes_inherit_entity_id', 'warunek zapisany w asercji musi być spełniony', 'All ID classes (suffix Id/IdRef) must inherit from EntityId, not ValueObject:\n' + '\n'.join(violations))
