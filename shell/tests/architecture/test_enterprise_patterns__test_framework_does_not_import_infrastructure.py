...
def test_framework_does_not_import_infrastructure() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_framework_does_not_import_infrastructure', 'warunek zapisany w asercji musi być spełniony', 'framework/ must not import from infrastructure/:\n' + '\n'.join(violations))
