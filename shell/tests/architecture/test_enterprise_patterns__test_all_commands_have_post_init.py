        ...
        for py_file in cmd_dir.rglob('*.py'):
            ...
            for node in ast.walk(tree):
                ...
                if key in _KNOWN_COMMANDS_NO_POST_INIT:
                    continue
                has_post_init = any(isinstance(m, ast.FunctionDef) and m.name == '__post_init__' for m in node.body)
                if not has_post_init:
                    missing.append(f'{py_file.relative_to(BASE)}: {node.name}')
    assert not missing, architecture_assertion_message('reguła testowana przez test_all_commands_have_post_init', 'warunek zapisany w asercji musi być spełniony', 'Command dataclasses must define __post_init__:\n' + '\n'.join(missing))
