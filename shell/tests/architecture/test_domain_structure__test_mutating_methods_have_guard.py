...
def test_mutating_methods_have_guard() -> None:
    violations: list[str] = []
    _NON_MUTATING = frozenset({'__init__', 'restore', 'pull_events', 'to_dict', 'new', 'create', 'generate', 'of', 'matches_trigger', 'get', 'snapshot', 'create_main_round', 'create_sub_graph', 'initialize', 'open'})
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_mutating_methods_have_guard', 'warunek zapisany w asercji musi być spełniony', 'Mutating methods in entities/aggregates should start with a guard clause (if ... raise):\n' + '\n'.join(violations))
