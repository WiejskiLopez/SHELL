...
_REGISTRIES: tuple[tuple[str, Callable[[], dict[str, type]]], ...] = (('definition', build_definition_event_registry), ('execution', build_execution_event_registry), ('ingestion', build_ingestion_event_registry), ('project', build_project_event_registry), ('scheduling', build_scheduling_event_registry), ('session', build_session_event_registry), ('user', build_user_event_registry))
    ...
    for bounded_context, build_registry in _REGISTRIES:
        ...
        for event_name, event_type in registry.items():
            if event_name != event_type.__name__:
                violations.append(f'{bounded_context}: key {event_name!r} does not match {event_type.__name__!r}')
            is_owned = event_type.__module__.startswith(expected_prefix)
            is_explicitly_consumed = any(event_type.__module__.startswith(allowed + '.') for allowed in _ALLOWED_CROSS_BC)
            if not is_owned and (not is_explicitly_consumed):
                violations.append(f'{bounded_context}: {event_type.__module__}.{event_name} is outside the owning BC and not an explicitly consumed cross-BC contract')
    assert not violations, architecture_assertion_message('reguła testowana przez test_each_bc_event_registry_contains_only_owned_events', 'warunek zapisany w asercji musi być spełniony', 'Invalid bounded-context event registries:\n' + '\n'.join(violations))
