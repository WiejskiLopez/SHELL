"""Architecture tests for bounded-context event registries."""
...
# Explicitly consumed cross-BC contracts (same allowlist as test_bc_isolation).
    ...
    for bounded_context, build_registry in _REGISTRIES:
        ...
        for event_name, event_type in registry.items():
            ...
            if not is_owned and not is_explicitly_consumed:
                violations.append(
                    f"{bounded_context}: {event_type.__module__}.{event_name} "
                    "is outside the owning BC and not an explicitly consumed cross-BC contract"
                )

    assert not violations, "Invalid bounded-context event registries:\n" + "\n".join(violations)
