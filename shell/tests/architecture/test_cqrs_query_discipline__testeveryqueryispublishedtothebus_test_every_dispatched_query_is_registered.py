    ...
    def test_every_dispatched_query_is_registered(self) -> None:
        ...
        for py_file in iter_py_files(SHELL_SRC):
            ...
            if tree is None:
                continue
            for node in ast.walk(tree):
                if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and (node.func.attr == 'dispatch') and node.args and isinstance(node.args[0], ast.Call):
        ...
        assert not missing, architecture_assertion_message('reguła testowana przez test_every_dispatched_query_is_registered', 'warunek zapisany w asercji musi być spełniony', 'Queries dispatched through the bus but never registered (runtime KeyError):\n' + '\n'.join(missing))
