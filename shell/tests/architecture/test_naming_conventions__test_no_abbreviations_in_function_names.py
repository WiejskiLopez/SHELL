...
def test_no_abbreviations_in_function_names() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_no_abbreviations_in_function_names', 'warunek zapisany w asercji musi być spełniony', 'Function/method names in production code must not use abbreviations:\n' + '\n'.join(violations))
