...
_STDLIB_NAMES: frozenset[str] = frozenset({'TYPE_CHECKING', 'Any', 'Protocol', 'Callable', 'Optional', 'Union', 'List', 'Dict', 'Tuple', 'Set', 'Iterable', 'Iterator', 'Generator', 'Sequence', 'Mapping', 'dataclass', 'field', 'datetime', 'timedelta', 'date', 'uuid', 'UUID', 'ABC', 'abstractmethod'})
...
def test_platform_value_objects_not_imported_from_definition() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_platform_value_objects_not_imported_from_definition', 'warunek zapisany w asercji musi być spełniony', '\n'.join(violations))
