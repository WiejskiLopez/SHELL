"""Koncept: reguła architektoniczna dotycząca aggregate scaffold: test aggregates have private new.

Reguła: test sprawdza kontrakt architektoniczny aggregate scaffold: test aggregates have private new.
...
def test_aggregates_have_private_new() -> None:
    ...
    assert not violations, architecture_assertion_message('reguła testowana przez test_aggregates_have_private_new', 'warunek zapisany w asercji musi być spełniony', 'Aggregates must have a private _new() factory:\n' + '\n'.join(violations))
