...
def test_future_annotations_in_every_file() -> None:
    ...
    for path in iter_py_files(BASE):
        ...
        if tree is None:
            continue
        has_future = any(isinstance(node, ast.ImportFrom) and node.module == '__future__' and any(alias.name == 'annotations' for alias in node.names) for node in ast.walk(tree))
        if not has_future:
            violations.append(rel)
    assert not violations, architecture_assertion_message('reguła testowana przez test_future_annotations_in_every_file', 'warunek zapisany w asercji musi być spełniony', 'Production .py files should have `from __future__ import annotations`:\n' + '\n'.join(violations))
