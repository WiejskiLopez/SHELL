...
class RagDocument(AggregateRoot[RagDocumentId]):
    ...
    def __init__(
        ...
        super().__init__(id)
        self._source_uri = (
            source_uri if isinstance(source_uri, SourceUri) else SourceUri(source_uri)
        )
    ...
    def add_chunks(
        ...
        self.append_event(
            RagDocumentChunksAddedEvent.now(
                self.id,
                chunk_count=ChunkIndex(len(chunk_ids)),
                model=model,
                now=(now or self._created_at),
            )
        )
