...
class WorkflowSkillAddedEvent(DomainEvent):
    workflow_id: WorkflowId
    skill_id: WorkflowSkillId

    @classmethod
    def now(cls, workflow_id: WorkflowId, skill_id: WorkflowSkillId, now: datetime) -> WorkflowSkillAddedEvent:
        return cls(occurred_at=now, workflow_id=workflow_id, skill_id=skill_id)
    ...
    def from_payload(cls, occurred_at: datetime, payload: dict[str, Any], schema_version: int = 1) -> Self:
        return cls(
            ...
            skill_id=WorkflowSkillId(payload.get("skill_id")),
        )
