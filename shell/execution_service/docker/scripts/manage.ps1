...
<#
.SYNOPSIS
    Zarzadza kontenerami mikroserwisu execution (api + worker) w projekcie shell-dev.
...
.DESCRIPTION
    ...
    Przyklady:
        .\docker\scripts\manage.ps1 up       # start kontenerow execution
        .\docker\scripts\manage.ps1 restart  # restart kontenerow execution
...
$targetServices = @("shell-execution-api", "shell-execution-worker")
$containerNames = @("shell-execution-shell-execution-api-1", "shell-execution-shell-execution-worker-1")

Write-Host "execution [$Action] ($Environment)" -ForegroundColor Cyan
...
switch ($Action) {
    ...
    "redeploy" { Remove-StaleContainers; & (Join-Path $projectRoot "scripts\prepare_mtls_ca.ps1"); if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; $env:CERTIFICATE_BUILD_ID = "execution-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmssfff'))-$PID"; & docker compose @composeArgs up -d --build @targetServices 2>&1 | Out-Host; exit $LASTEXITCODE }
