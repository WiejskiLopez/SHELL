...
from shell.execution_service.infrastructure.execution.session_execution.adapters.session_query_provider.mappers.session_response_to_session_reference import (
...
class SessionQueryProviderHttpAdapter(SessionQueryProvider):
    ...
    async def get_by_id(self, session_id: SessionIdRef) -> SessionReference | None:
        response = await self._client.get(f"/api/v1/sessions/{session_id.value}/history")
