...
<#
.SYNOPSIS
    Zarzadza kontenerami mikroserwisu definition (api + worker) w projekcie shell-dev.
...
.DESCRIPTION
    ...
    Przyklady:
        .\docker\scripts\manage.ps1 up       # start kontenerow definition
        .\docker\scripts\manage.ps1 restart  # restart kontenerow definition
...
$targetServices = @("shell-definition-api", "shell-definition-worker")
$containerNames = @("shell-definition-shell-definition-api-1", "shell-definition-shell-definition-worker-1")

Write-Host "definition [$Action] ($Environment)" -ForegroundColor Cyan
...
switch ($Action) {
    ...
    "redeploy" { Remove-StaleContainers; & (Join-Path $projectRoot "scripts\prepare_mtls_ca.ps1"); if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; $env:CERTIFICATE_BUILD_ID = "definition-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmssfff'))-$PID"; & docker compose @composeArgs up -d --build @targetServices 2>&1 | Out-Host; exit $LASTEXITCODE }
