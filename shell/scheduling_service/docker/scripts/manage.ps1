...
<#
.SYNOPSIS
    Zarzadza kontenerami mikroserwisu scheduling (api + worker) w projekcie shell-dev.
...
.DESCRIPTION
    ...
    Przyklady:
        .\docker\scripts\manage.ps1 up       # start kontenerow scheduling
        .\docker\scripts\manage.ps1 restart  # restart kontenerow scheduling
...
$targetServices = @("shell-scheduling-api", "shell-scheduling-worker")
$containerNames = @("shell-scheduling-shell-scheduling-api-1", "shell-scheduling-shell-scheduling-worker-1")

Write-Host "scheduling [$Action] ($Environment)" -ForegroundColor Cyan
...
switch ($Action) {
    ...
    "redeploy" { Remove-StaleContainers; & (Join-Path $projectRoot "scripts\prepare_mtls_ca.ps1"); if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; $env:CERTIFICATE_BUILD_ID = "scheduling-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmssfff'))-$PID"; & docker compose @composeArgs up -d --build @targetServices 2>&1 | Out-Host; exit $LASTEXITCODE }
