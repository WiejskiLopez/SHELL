...
if TYPE_CHECKING:
    ...
    from shell.application.platform.ports.queries import RunnerConfigQueryService  # type: ignore[attr-defined]
...
class RunnerConfigGetHandler:
    ...
    async def handle(self, get_runner_config_query: RunnerConfigGetQuery) -> RunnerConfigDto | None:
        return await self._queries.get_runner_config(get_runner_config_query.package_name)  # type: ignore[no-any-return]
