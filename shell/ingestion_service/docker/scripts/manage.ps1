...
<#
.SYNOPSIS
    Zarzadza kontenerami mikroserwisu ingestion (api + worker) w projekcie shell-dev.
...
.DESCRIPTION
    ...
    Przyklady:
        .\docker\scripts\manage.ps1 up       # start kontenerow ingestion
        .\docker\scripts\manage.ps1 restart  # restart kontenerow ingestion
...
$targetServices = @("shell-ingestion-api", "shell-ingestion-worker")
$containerNames = @("shell-ingestion-shell-ingestion-api-1", "shell-ingestion-shell-ingestion-worker-1")

Write-Host "ingestion [$Action] ($Environment)" -ForegroundColor Cyan
...
switch ($Action) {
    ...
    "redeploy" { Remove-StaleContainers; & (Join-Path $projectRoot "scripts\prepare_mtls_ca.ps1"); if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; $env:CERTIFICATE_BUILD_ID = "ingestion-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmssfff'))-$PID"; & docker compose @composeArgs up -d --build @targetServices 2>&1 | Out-Host; exit $LASTEXITCODE }
