...
class GraphExecutionSagaState:
    ...
    graph_node_definition_executions: dict[str, str] = field(default_factory=dict)
    ...
    def record_node_execution_created(
        ...
        self.graph_node_definition_executions[node_definition_id] = node_execution_id
        if self.is_complete:
            self.status = GraphExecutionSagaStatus.COMPLETED

    @property
    def is_complete(self) -> bool:
        return len(self.graph_node_definition_executions) >= self.expected_nodes_count
