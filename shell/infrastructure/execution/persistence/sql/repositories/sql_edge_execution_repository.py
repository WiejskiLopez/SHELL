...
def _model_to_entity(model: EdgeExecutionModel) -> EdgeExecution:
    return EdgeExecution.restore(
        ...
        source_node_execution_id=(
            NodeExecutionId(model.source_node_execution_id)
        ),
        ...
        deleted_at=DeletedAt.from_datetime(model.deleted_at) if model.deleted_at is not None else None,
    )
