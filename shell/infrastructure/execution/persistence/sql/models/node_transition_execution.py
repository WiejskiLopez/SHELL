...
if TYPE_CHECKING:
    from shell.infrastructure.execution.persistence.sql.models.graph_execution import (  # noqa: E402 — łamie circular import NodeTransitionExecutionModel ↔ GraphExecutionModel
        GraphExecutionModel,  # noqa: TC002 — GraphExecutionModel używany w Mapped[GraphExecutionModel] w relacji SQLAlchemy
    )


class NodeTransitionExecutionModel(Base, VersionedMixin):
    __tablename__ = "node_transition_execution"
    ...
    graph_execution_model: Mapped[GraphExecutionModel] = relationship(
        back_populates="node_transition_execution_models",
    )
