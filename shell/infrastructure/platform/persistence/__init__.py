...
def __getattr__(name: str) -> object:
    if name == "SqlAlchemyUnitOfWork":
        from shell.infrastructure.platform.persistence.sql_alchemy_uow import (  # noqa: PLC0415 -- deferred to break circular import chain
            SqlAlchemyUnitOfWork,
        )

        return SqlAlchemyUnitOfWork
    if name == "SqlAlchemyUnitOfWorkBase":
        from shell.infrastructure.platform.persistence.sql_alchemy_uow_base import (  # noqa: PLC0415 -- deferred to break circular import chain
            SqlAlchemyUnitOfWorkBase,
        )

        return SqlAlchemyUnitOfWorkBase
    if name == "InMemoryRepository":
        from shell.infrastructure.platform.persistence.in_memory_repository import (  # noqa: PLC0415 -- deferred to break circular import chain
            InMemoryRepository,
        )
