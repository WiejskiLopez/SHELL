...
def upgrade() -> None:
    ...
    op.create_index(
        "uq_task_execution_state_task_id_kind_current",
        "task_execution_state",
        ["task_execution_id", "kind"],
    ...
    op.execute(
        """INSERT INTO task_execution_state (id, task_execution_id, kind, payload, is_current, created_at)
...
def downgrade() -> None:
    ...
    op.execute(
        """INSERT INTO task_execution_state_input (id, task_execution_id, payload, is_current, created_at)
           SELECT id, task_execution_id, payload, is_current, created_at
           FROM task_execution_state WHERE kind = 'INPUT'"""
    )
    op.execute(
        """INSERT INTO task_execution_state_output (id, task_execution_id, payload, is_current, created_at)
           SELECT id, task_execution_id, payload, is_current, created_at
           FROM task_execution_state WHERE kind = 'OUTPUT'"""
    )

    op.drop_index(
        "uq_task_execution_state_task_id_kind_current", table_name="task_execution_state"
    )
    op.drop_index(
