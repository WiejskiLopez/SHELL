"""Phase 22 — split graph_node_transition into execution and definition tables.

Revision ID: 014
Revises: 013
Create Date: 2026-06-19

* Rename ``graph_node_transition`` → ``graph_node_transition_execution``
  (execution-side transitions, referencing graph_execution / graph_node_execution).
* Create ``graph_node_transition_definition`` table — template transitions
  referencing graph_definition / graph_node_definition.
"""
...
def upgrade() -> None:
    op.rename_table("graph_node_transition", "graph_node_transition_execution")
...
def downgrade() -> None:
    op.drop_table("graph_node_transition_definition")
    op.rename_table("graph_node_transition_execution", "graph_node_transition")
