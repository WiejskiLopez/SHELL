...
class SqlRagDocumentRepository(RagDocumentRepository):
    ...
    async def delete(self, id: RagDocumentId) -> None:
        await self._session.execute(
            sa_delete(RagChunkModel).where(RagChunkModel.document_id == id.value)
        )
        await self._session.execute(
            sa_delete(RagDocumentModel).where(RagDocumentModel.id == id.value)
        )

    async def exists(self, id: RagDocumentId) -> ExistsResult:
        row = await self._session.get(RagDocumentModel, id.value)
        return ExistsResult(value=row is not None)
