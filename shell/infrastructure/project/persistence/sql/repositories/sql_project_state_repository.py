...
class SqlProjectStateRepository(ProjectStateRepository):
    ...
    async def get_by_id(
        self, project_state_id: ProjectStateId
    ) -> ProjectState | None:
    ...
    async def save(self, state: ProjectState) -> None:
        ...
        if model is None:
            ...
            model.state_data = state.snapshot()
            model.is_current = state.is_current
            model.updated_at = (
                state.updated_at.value if state.updated_at else None
            )
