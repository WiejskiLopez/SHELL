...
class SchedulingCoreContainer(containers.DeclarativeContainer):
    ...
    command_registry = providers.Object(
        build_command_registry(
            discover_command_types("shell.scheduling.application.scheduling")
        )
    )
    ...
    rabbit_command_inbox_consumer_factory = providers.Factory(
        ...
        session_factory=session_factory,
        models=persistence_delivery_models.provided.commands,
        queue_name="shell-scheduling-command-inbox",
    ...
    update_scheduler_job_handler_factory = providers.Factory(
        UpdateSchedulerJobHandler, unit_of_work=scheduler_execution_uow_factory, clock=clock_factory
    )
    delete_scheduler_job_handler_factory = providers.Factory(
        DeleteSchedulerJobHandler, unit_of_work=scheduler_execution_uow_factory, clock=clock_factory
    )
