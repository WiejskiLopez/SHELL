"""Optimistic locking tests for Workflow aggregate."""

from __future__ import annotations

from datetime import datetime
from typing import TYPE_CHECKING

import pytest
from shell.domain.execution.aggregates.workflow.workflow import Workflow
from shell.domain.execution.aggregates.workflow.value_objects.workflow_id import WorkflowId
from shell.domain.platform.exceptions.concurrent_modification_error import (
    ConcurrentModificationError,
)
from shell.infrastructure.platform.persistence import SqlAlchemyUnitOfWork

if TYPE_CHECKING:
    from sqlalchemy.ext.asyncio import async_sessionmaker


_NOW = datetime(2024, 1, 1, 12, 0, 0)


class TestWorkflowOptimisticLocking:
    async def test_concurrent_modification_raises_error(
        self,
        session_factory: async_sessionmaker,
    ) -> None:
        uow1 = SqlAlchemyUnitOfWork(session_factory)
        uow2 = SqlAlchemyUnitOfWork(session_factory)

        async with uow1 as u:
            workflow = Workflow.new(id_=WorkflowId.generate(), now=_NOW)
            await u.workflow_repository.save(workflow)
            await u.commit()
            wf_id = workflow.id

        async with uow1 as u1:
            entity_a = await u1.workflow_repository.get_by_id(wf_id)
            assert entity_a is not None

            async with uow2 as u2:
                entity_b = await u2.workflow_repository.get_by_id(wf_id)
                assert entity_b is not None

                entity_a.finish(now=_NOW)
                await u1.workflow_repository.save(entity_a)
                await u1.commit()

                entity_b.abort(reason="concurrent", now=_NOW)
                await u2.workflow_repository.save(entity_b)
                with pytest.raises(ConcurrentModificationError):
                    await u2.commit()
